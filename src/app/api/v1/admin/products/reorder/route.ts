...
async function reorderProductsHandler(request: NextRequest, user: JwtData) {
  if (user.role !== 'ADMIN') {
  ...
  try {
    ...
    // Update all products with new sort orders
    await Promise.all(
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to reorder products', error: error.message },
      { status: 500 }
    );
  }
}

export const POST = requireAuth(reorderProductsHandler);
