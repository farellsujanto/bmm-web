...
export default function ActivityContent() {
  ...
  useEffect(() => {
    if (isAuthenticated && !authLoading && user?.role !== 'ADMIN' && !loadedTabs.has(activeTab)) {
      fetchTabData(activeTab);
    }
  }, [activeTab, isAuthenticated, authLoading, user, loadedTabs]);
  ...
  const fetchTabData = async (tab: string) => {
    try {
      ...
      if (tab === 'overview') {
        ...
        const [affiliateResData, profileData] = await Promise.all([
          apiRequest.get('/v1/user/affiliate'),
          userData ? Promise.resolve({ success: true, data: userData }) : apiRequest.get('/v1/user/profile')
        ]);
        ...
        const missionsData = await apiRequest.get('/v1/user/missions');
        if (missionsData.success) {
          setMissions(missionsData.data as any[]);
        }
      }
